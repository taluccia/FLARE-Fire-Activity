...
# Process point to polygon for 2003 
...
### Process 2003 over 3 regions
```{r}
eco_nepoly1_ee_sf = st_as_sf(eco_nepoly1_ee)
eco_E_EPoly2_1_ee_sf = st_as_sf(eco_E_EPoly2_1_ee)
eco_W_EPoly2_2_ee_sf = st_as_sf(eco_W_EPoly2_2_ee)
```

### NE ploy
```{r}
eco1_hs = hs_ee_sf[eco_nepoly1_ee_sf, ]

remove_issue1_pt = eco1_hs %>% filter(LONGITUDE != 179.9466)

eco1_hs_3k = st_buffer(remove_issue1_pt, dist = 3000, endCapStyle = "ROUND", joinStyle = "ROUND",) # Buffer by 3k

eco1_hs_3k_nh = nngeo::st_remove_holes(eco1_hs_3k)

eco1_hs_3k_nh_wgs = st_transform(eco1_hs_3k_nh, crs=wgs_proj)

```

```{r eval=FALSE, include=FALSE}
st_write(eco1_hs_3k_nh_wgs, "../outputs/hotspot-poly-single-eco/hspoly_NE_Eco_2003.shp", driver="ESRI Shapefile")
```
...
```{r}
eco2_hs = hs_ee_sf[eco_E_EPoly2_1_ee_sf, ]

remove_issue2_pt = eco2_hs %>% filter(LONGITUDE != 179.9466)

eco2_hs_3k = st_buffer(remove_issue2_pt, dist = 3000, endCapStyle = "ROUND", joinStyle = "ROUND",) # Buffer by 3k

eco2_hs_3k_nh = nngeo::st_remove_holes(eco2_hs_3k)

eco2_hs_3k_nh_wgs = st_transform(eco2_hs_3k_nh, crs=wgs_proj)

```

```{r eval=FALSE, include=FALSE}
st_write(eco2_hs_3k_nh_wgs, "../outputs/hotspot-poly-single-eco/hspoly_EEPOLY_Eco_2003.shp", driver="ESRI Shapefile")
```

### For western portion of E Poly 2

```{r}
eco3_hs = hs_ee_sf[eco_W_EPoly2_2_ee_sf, ]

remove_issue3_pt = eco3_hs %>% filter(LONGITUDE != 179.9466)

eco3_hs_3k = st_buffer(remove_issue3_pt, dist = 3000, endCapStyle = "ROUND", joinStyle = "ROUND",) # Buffer by 3k

eco3_hs_3k_nh = nngeo::st_remove_holes(eco3_hs_3k)

eco3_hs_3k_nh_wgs = st_transform(eco3_hs_3k_nh, crs=wgs_proj)
...
```{r eval=FALSE, include=FALSE}
st_write(eco3_hs_3k_nh_wgs, "../outputs/hotspot-poly-single-eco/hspoly_WEPoly_Eco_2003.shp", driver="ESRI Shapefile")
```


# Process Point to Polygon for Large data year 2019 & 2020)
