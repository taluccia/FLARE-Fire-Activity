...
```{r}
fires_at_eco = st_join(fires_at, ecozones_ee, largest=TRUE) %>% dplyr::select(FID:AREA, ECO_NAME:ECO_SYM, eco_code:geometry) %>% drop_na(eco_code)
```
...
```{r}
fires_at_eco_arc %>% group_by(ID_obj) %>% filter(n()>1) %>% summarize(n=n())
```
```{r}
fires_at_eco_arc %>% group_by(ID_obj) %>% filter(ID_obj=="233") 
```
...
```{r eval=FALSE, include=FALSE}
st_write(fires_at_eco_arc_hs_merge, "../outputs/test/test_attributes.shp", driver="ESRI Shapefile") 
