---
title: "JuneJulyClimateFireAnalysis"
author: "Anna Talucci"
date: "6/17/2021"
...
# Analysis  June Variables
## Model Climate Moisture Deficit
```{r}
fit1 = lm(MHa ~ JuneDef + EcoCode + JuneDef:EcoCode, data = climate)
```
#### Residuals
...
```{r}
qplot(x = JuneDef, y = res1, data = climate,
xlab = "CMD",
ylab = "Residuals",
...
```{r}
 climate %>% 
  nest(-EcoCode) %>% 
  mutate(fit = map(data, ~ lm(MHa ~ JuneDef, data = .)),
...
plot_fit1 = ggplot(data = climate, aes(x = JuneDef, y = MHa)) + 
  geom_smooth(method="lm", se=TRUE, alpha=0.1) +
  geom_point(size = 1) +
  scale_color_manual(values=cbbPalette) +
    labs(y=expression(atop("MHa")), x= "Climate Moisture Deficit") +
...
ggsave("../figures/JuneDefAnalysis.png", plot = plot_fit1, width = 6, height = 7, units = c("in"), dpi=600 )
```
...
```{r}
fit2 = lm(MHa ~ JunePr + EcoCode + JunePr:EcoCode, data = climate)
```
#### Residuals
...
```{r}
qplot(x = JunePr, y = res2, data = climate,
xlab = "CMD",
ylab = "Residuals",
main = "Residuals vs CMD")
```
...
```{r}
 climate %>% 
  nest(-EcoCode) %>% 
  mutate(fit = map(data, ~ lm(MHa ~ JunePr, data = .)),
         results = map(fit, glance)) %>% 
  unnest(results)
```
...
```{r fig.height=7, fig.width=6}
plot_fit2 = ggplot(data = climate, aes(x = JunePr, y = MHa)) + 
  geom_smooth(method="lm", se=TRUE, alpha=0.1) +
  geom_point(size = 1) +
  scale_color_manual(values=cbbPalette) +
    labs(y=expression(atop("MHa")), x= "Precipitation") +
...
```{r eval=FALSE, include=FALSE}
ggsave("../figures/JunePrAnalysis.png", plot = plot_fit2, width = 6, height = 7, units = c("in"), dpi=600 )
```
...
```{r}
fit3 = lm(MHa ~ JuneTmmx + EcoCode + JuneTmmx:EcoCode, data = climate)
```
#### Residuals
...
```{r}
qplot(x = JuneTmmx, y = res2, data = climate,
xlab = "CMD",
ylab = "Residuals",
main = "Residuals vs CMD")
```
...
```{r}
 climate %>% 
  nest(-EcoCode) %>% 
  mutate(fit = map(data, ~ lm(MHa ~ JuneTmmx, data = .)),
         results = map(fit, glance)) %>% 
  unnest(results)
```
...
```{r fig.height=7, fig.width=6}
plot_fit3 = ggplot(data = climate, aes(x = JuneTmmx, y = MHa)) + 
  geom_smooth(method="lm", se=TRUE, alpha=0.1) +
  geom_point(size = 1) +
  scale_color_manual(values=cbbPalette) +
    labs(y=expression(atop("MHa")), x= "Max Temp") +
...
```{r eval=FALSE, include=FALSE}
ggsave("../figures/JuneTmmxAnalysis.png", plot = plot_fit3, width = 6, height = 7, units = c("in"), dpi=600 )
```


# Analysis  July Variables
## Model Climate Moisture Deficit
```{r}
fit4 = lm(MHa ~ JulyDef + EcoCode + JulyDef:EcoCode, data = climate)
```
#### Residuals
...
```{r}
# Get the fitted value for each observation
climate$fit4 = fitted(fit4)
```

```{r}
# Get the residuals of the model
climate$res1 = residuals(fit4)
```

```{r}
qplot(x = fit4, y = res1, data = climate,
main = "Residuals vs Fitted Values")
```

```{r}
qplot(x = JulyDef, y = res1, data = climate,
xlab = "CMD",
ylab = "Residuals",
main = "Residuals vs CMD")
```
...
```{r}
 climate %>% 
  nest(-EcoCode) %>% 
  mutate(fit = map(data, ~ lm(MHa ~ JulyDef, data = .)),
         results = map(fit, glance)) %>% 
  unnest(results)
```
...
```{r fig.height=7, fig.width=6}
plot_fit4 = ggplot(data = climate, aes(x = JulyDef, y = MHa)) + 
  geom_smooth(method="lm", se=TRUE, alpha=0.1) +
  geom_point(size = 1) +
  scale_color_manual(values=cbbPalette) +
    labs(y=expression(atop("MHa")), x= "Climate Moisture Deficit") +
...
```{r eval=FALSE, include=FALSE}
ggsave("../figures/JulyDefAnalysis.png", plot = plot_fit4, width = 6, height = 7, units = c("in"), dpi=600 )
```
...
```{r}
fit5 = lm(MHa ~ JulyPr + EcoCode + JulyPr:EcoCode, data = climate)
```
#### Residuals
...
```{r}
# Get the fitted value for each observation
climate$fit5 = fitted(fit5)
```

```{r}
# Get the residuals of the model
climate$res2 = residuals(fit5)
```

```{r}
qplot(x = fit5, y = res2, data = climate,
main = "Residuals vs Fitted Values")
```

```{r}
qplot(x = JulyPr, y = res2, data = climate,
xlab = "CMD",
ylab = "Residuals",
main = "Residuals vs CMD")
```
...
```{r}
 climate %>% 
  nest(-EcoCode) %>% 
  mutate(fit = map(data, ~ lm(MHa ~ JulyPr, data = .)),
         results = map(fit, glance)) %>% 
  unnest(results)
```
...
```{r fig.height=7, fig.width=6}
plot_fit5 = ggplot(data = climate, aes(x = JulyPr, y = MHa)) + 
  geom_smooth(method="lm", se=TRUE, alpha=0.1) +
  geom_point(size = 1) +
  scale_color_manual(values=cbbPalette) +
    labs(y=expression(atop("MHa")), x= "Precipitation") +
...
```{r eval=FALSE, include=FALSE}
ggsave("../figures/JulyPrAnalysis.png", plot = plot_fit5, width = 6, height = 7, units = c("in"), dpi=600 )
```

## Model Max Temperature

```{r}
fit6 = lm(MHa ~ JulyTmmx + EcoCode + JulyTmmx:EcoCode, data = climate)
```
#### Residuals
...
```{r}
# Get the fitted value for each observation
climate$fit6 = fitted(fit6)
```

```{r}
# Get the residuals of the model
climate$res2 = residuals(fit6)
```

```{r}
qplot(x = fit6, y = res2, data = climate,
main = "Residuals vs Fitted Values")
```

```{r}
qplot(x = JulyTmmx, y = res2, data = climate,
xlab = "CMD",
ylab = "Residuals",
main = "Residuals vs CMD")
```
...
```{r}
plot(fit6, which = 1) # residual vs fitted values
```

```{r}
plot(fit6, which = 2) # qqnorm plot of residuals
```
...
```{r}
int_BT = coef(fit6)[1]
int_CKMT = coef(fit6)[1] + coef(fit6)[3]
int_CPT = coef(fit6)[1] + coef(fit6)[4]
int_EST = coef(fit6)[1] + coef(fit6)[5]
int_NSCT = coef(fit6)[1] + coef(fit6)[6]
int_NST = coef(fit6)[1] + coef(fit6)[7]
int_TBBMT = coef(fit6)[1] + coef(fit6)[8]
int_TCST = coef(fit6)[1] + coef(fit6)[9]
...
slope_BT = coef(fit6)[2]
slope_CKMT = coef(fit6)[2] + coef(fit6)[10]
slope_CPT = coef(fit6)[2] + coef(fit6)[11]
slope_EST = coef(fit6)[2] + coef(fit6)[12]
slope_NSCT = coef(fit6)[2] + coef(fit6)[13]
slope_NST = coef(fit6)[2] + coef(fit6)[14]
slope_TBBMT = coef(fit6)[2] + coef(fit6)[15]
slope_TCST = coef(fit6)[2] + coef(fit6)[16]
...
```{r}
 climate %>% 
  nest(-EcoCode) %>% 
  mutate(fit = map(data, ~ lm(MHa ~ JulyTmmx, data = .)),
         results = map(fit, glance)) %>% 
  unnest(results)
```
...
```{r fig.height=7, fig.width=6}
plot_fit6 = ggplot(data = climate, aes(x = JulyTmmx, y = MHa)) + 
  geom_smooth(method="lm", se=TRUE, alpha=0.1) +
  geom_point(size = 1) +
  scale_color_manual(values=cbbPalette) +
    labs(y=expression(atop("MHa")), x= "Max Temp") +
...
```{r eval=FALSE, include=FALSE}
ggsave("../figures/JulyTmmxAnalysis.png", plot = plot_fit6, width = 6, height = 7, units = c("in"), dpi=600 )
```
