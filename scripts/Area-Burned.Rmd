...
plot_frq_arctic_nl = ggplot(fire_frq_arctic, aes(x=fire_yr, y=n_fires, fill=location)) +
  ...
  scale_y_continuous(name = ('Frequency \n(count)'), breaks=c(0, 2000000, 4000000, 6000000, 8000000, 10000000), labels = c(0, 2, 4, 6, 8, 10)) +
  

  theme_bw() + theme(legend.position = "none") + 
  scale_fill_manual(values=arctic_palette, name="", labels = c("Arctic", "Subarctic")) +
  xlab('') +
  theme(panel.grid.major = element_blank()) +
    theme(axis.title.y = element_text(size = 11, hjust = 0.5, vjust = 1.1),
        axis.text.x = element_text(size = 10, color = "black"),
...
```
```{r}
arctic_palette = c("#018571", "#a6611a")
```
...
```{r eval=FALSE, include=FALSE}
ggsave("../figures/2021-01-07_ArctisAreaBurned.png", plot = plot_area, width = 6, height = 4, units = c("in"), dpi=600 )
```
...
```{r fig.height=7, fig.width=6}
cowplot::plot_grid(plot_area, plot_frq, area_class_no_legend, frq_class_no_legend, plot_area_arctic_nl, plot_frq_arctic_nl, labels = "AUTO", ncol = 2)
