...
```{r}
f01 %>% mutate(location = ifelse(geometry %in% 66.5:75, "arctic", "subarctic"))
```

```{r}
st_filter(f01, arctic, y)
```


```{r}
f01_arctic = st_join(f01, arctic)
head(f01_arctic)
```

```{r}
perimeters2001 = f01_arctic %>% dplyr::mutate(location = replace_na(location, "subarctic"))
