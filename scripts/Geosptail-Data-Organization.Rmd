...
poly_folder <- "../data/clean-shapefile/flare_30m/" 
    ...
    poly_shp_files
poly_names <-list(substr(poly_shp_files,1,19))
poly_names
    #--inspect list
    #print(shp_files)
```
CityA_CD.shp fires2017.shp
```{r}
...
filenames <- list.files(poly_folder , pattern="\\.shp$", full.names=TRUE)
```
...
```{r}
mylist <- makeNamedList(shapefile_list)
str(mylist[[1]])
```
...
```{r}
mapply(f_intersect(point_shp_list, poly_shp_list)) %>% f1()
```
