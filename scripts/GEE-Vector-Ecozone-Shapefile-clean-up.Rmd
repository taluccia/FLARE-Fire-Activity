...
# Read in shapefile
```{r}
E_EPoly2_1 = st_read("../data/part2-gee-vectors-EqualArea/E_EPoly2_1_ee_fires90_2001.shp", "E_EPoly2_1_ee_fires90_2001") 
NEPoly1 = st_read("../data/part2-gee-vectors-EqualArea/NEPoly1_ee_fires90_2001.shp", "NEPoly1_ee_fires90_2001") 
W_EPoly2_2 = st_read("../data/part2-gee-vectors-EqualArea/W_EPoly2_2_ee_fires90_2001.shp", "W_EPoly2_2_ee_fires90_2001") 

```

```{r eval=FALSE, include=FALSE}
ckmt_nst_nsct = st_read("../data/part2-gee-vectors-SingleEco/ckmt_nst_nsct_fires_2003.shp", "ckmt_nst_nsct_fires_2003") 
cpt_bt_east = st_read("../data/part2-gee-vectors-SingleEco/cpt_bt_east_fires_2003.shp", "cpt_bt_east_fires_2003") 
cpt_bt_west = st_read("../data/part2-gee-vectors-SingleEco/cpt_bt_west_fires_2003.shp", "cpt_bt_west_fires_2003") 
est_tbbmt_n = st_read("../data/part2-gee-vectors-SingleEco/est_tbbmt_n_fires_2003.shp", "est_tbbmt_n_fires_2003") 
est_tbbmt_s = st_read("../data/part2-gee-vectors-SingleEco/est_tbbmt_s_fires_2003.shp", "est_tbbmt_s_fires_2003") 
tcst_n = st_read("../data/part2-gee-vectors-SingleEco/tcst_n_fires_2003.shp", "tcst_n_fires_2003") 
tcst_s = st_read("../data/part2-gee-vectors-SingleEco/tcst_s_fires_2003.shp", "tcst_s_fires_2003") 
```

# Equal Area
```{r}
E_EPoly2_1_ee = st_transform(E_EPoly2_1, crs=ee_russia)
NEPoly1_ee = st_transform(NEPoly1, crs=ee_russia)
W_EPoly2_2_ee = st_transform(W_EPoly2_2, crs=ee_russia)
```
...
# Buffer (500 m)
```{r}
E_EPoly2_1_buff =st_buffer(E_EPoly2_1_ee, dist=500, endCapStyle = "ROUND", joinStyle = "ROUND")
```

```{r}
NEPoly1_buff =st_buffer(NEPoly1_ee, dist=500, endCapStyle = "ROUND", joinStyle = "ROUND")
```

```{r}
W_EPoly2_2_buff =st_buffer(W_EPoly2_2_ee, dist=500, endCapStyle = "ROUND", joinStyle = "ROUND")
```

# Remove holes
```{r}
E_EPoly2_1_buff_nh = nngeo::st_remove_holes(E_EPoly2_1_buff)
```

```{r}
NEPoly1_buff_nh = nngeo::st_remove_holes(NEPoly1_buff)
```

```{r}
W_EPoly2_2_buff_nh = nngeo::st_remove_holes(W_EPoly2_2_buff)
```
# Combine into one shapefile
```{r}
all_shp1 = rbind(E_EPoly2_1_buff, NEPoly1_buff, W_EPoly2_2_buff)
all_shp1
```
```{r}
...
```{r}
all_shp_un_poly_nh = nngeo::st_remove_holes(all_shp_un_poly, max_area = 0)
```

```{r}
st_write(all_shp_un_poly, "../outputs/GEE-clean-shapefiles-EqualArea/combine_ecozones_ea90_2001.shp", driver="ESRI Shapefile") 
```
