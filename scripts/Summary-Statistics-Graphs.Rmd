...
```{r}
YrEco = data %>% group_by(FireYr, EcoName, EcoCode) %>%
...
fig1 = ggplot(YrEco, aes(x=FireYr, y=TotHa, fill=EcoCode)) +
...
ptcEco
```{r}
pctEco = data %>% dplyr::select(FireYr, SizeKm2, EcoName, EcoCode, Ecokm2) %>%
...
fig2 = ggplot(pctEco, aes(x=FireYr, y=PctEco)) +
  geom_bar(stat="identity", width = 0.5, position = position_stack(reverse = FALSE)) +
...
```{r}
clar = data %>% dplyr::select(FireYr, SizeHa, EcoName, EcoCode, Class) %>%
...
fig3 = ggplot(clar, aes(x=FireYr, y=milha, fill=Class)) +
  geom_bar(stat="identity", width = 0.5, position = position_stack(reverse = FALSE)) +
  facet_wrap(~EcoCode, scales = "free_y") +
...
```{r}
ggplot(data, x=TotDays, y=milha, aes(fill=Class)) +
